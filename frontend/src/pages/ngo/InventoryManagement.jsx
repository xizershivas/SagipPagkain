    ...
    <DashboardLayout title="Inventory Management">
      <div className="data-table-container">
        <div className="data-table-header">
          <h6><i className="bi bi-box-seam me-2" style={{ color: 'var(--primary)' }}></i>My Food Bank Inventory</h6>
        ...
        <div className="table-responsive">
          <table className="table table-hover mb-0">
            <thead>
              <tr><th>Item</th><th>Category</th><th>Quantity</th><th>Unit</th><th>Expiry</th><th>Status</th></tr>
            </thead>
            <tbody>
              {loading ? (
                <tr><td colSpan={6} className="text-center py-4"><div className="spinner-border spinner-border-sm"></div></td></tr>
              ) : inventory.length === 0 ? (
                <tr><td colSpan={6} className="text-center py-4 text-muted">No inventory records</td></tr>
              ) : inventory.map(i => {
                const today = new Date();
                const expiry = new Date(i.dtmExpirationDate);
